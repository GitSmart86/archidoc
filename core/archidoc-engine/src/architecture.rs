...
use archidoc_types::{C4Level, HealthStatus, ModuleDoc};
...
///
/// `columns` controls which columns appear in the Component Index table.
/// Pass an empty slice to use the default columns (Module, Level, Pattern, Description).
/// Column names are matched case-insensitively to IR fields.
///
/// Sections:
...
/// 5. Relationship Map
/// 6. Footer
pub fn generate(docs: &[ModuleDoc], root: &Path, columns: &[String]) -> String {
    ...
    output.push_str(&section_component_index(docs, root, columns));
...
fn section_component_index(docs: &[ModuleDoc], root: &Path, columns: &[String]) -> String {
    ...
    for doc in &modules {
        let cells: Vec<String> = cols
            .iter()
            .map(|col| resolve_column(col, doc, root))
            .collect();
        output.push_str(&format!("| {} |\n", cells.join(" | ")));
    }
