...
enum Commands {
    ...
    ///
    /// Two variants:
    ///   archidoc tree .              → _context/ARCHITECTURE.tree.md  (dirs only)
    ///   archidoc tree . --files      → _context/ARCHITECTURE.files.md (dirs + files)
    ///   archidoc tree . --both       → generates both files in one pass
    ///
    ...
    Tree {
        ...
        /// Include files under each directory (outputs ARCHITECTURE.files.md)
        #[arg(long)]
        files: bool,

        /// Generate both dirs-only and dirs+files variants in one pass
...
fn main() {
    ...
    if let Some(command) = cli.command {
        match command {
            ...
            Commands::Tree { path, files, both, depth, out } => {
                run_tree(path.as_deref(), files, both, depth, &out);
                return;
            }
            Commands::Audit { path } => {
...
fn run_tree(
    ...
    if emit_dirs {
        let content = format!(
            ...
            archidoc_engine::tree::dirs_tree(&root, depth)
        );
    ...
    if emit_files {
        let content = format!(
            "# File Tree\n\n> Auto-generated by archidoc. Do not edit manually.\n\n```\n{}```\n",
            archidoc_engine::tree::files_tree(&root, depth)
        );
        let dest = out_path.join("ARCHITECTURE.files.md");
        fs::write(&dest, &content).unwrap_or_else(|e| {
            eprintln!("error: failed to write {}: {}", dest.display(), e);
            std::process::exit(1);
        });
